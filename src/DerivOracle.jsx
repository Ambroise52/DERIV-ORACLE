...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
  ...
  .mesh-state-badge{display:inline-flex;align-items:center;gap:8px;padding:6px 14px;border-radius:3px;font-family:var(--head);font-size:11px;letter-spacing:3px;font-weight:700;}
  .mesh-state-IDLE{background:rgba(74,82,96,0.2);border:1px solid #4a5260;color:#4a5260;}
  .mesh-state-ANALYZING{background:var(--yellow-dim);border:1px solid var(--yellow);color:var(--yellow);}
  .mesh-state-EXECUTING{background:var(--green-dim);border:1px solid var(--green);color:var(--green);}
  .mesh-state-PAUSED_DRIFT{background:var(--orange-dim);border:1px solid var(--orange);color:var(--orange);}
  .mesh-state-ERROR{background:var(--red-dim);border:1px solid var(--red);color:var(--red);}
  .mesh-signal-EXECUTE{background:var(--green-dim);border:2px solid var(--green);color:var(--green);}
  .mesh-signal-PAUSED_DRIFT{background:var(--orange-dim);border:2px solid var(--orange);color:var(--orange);}
  .mesh-signal-IDLE{background:rgba(74,82,96,0.1);border:1px solid #4a5260;color:#4a5260;}
  .mesh-config-row{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px;}
  .mesh-config-item{padding:8px;background:var(--bg2);border:1px solid var(--border);border-radius:3px;}
  .mesh-config-label{font-size:8px;letter-spacing:2px;color:var(--text-dim);margin-bottom:4px;}
  .mesh-config-input{width:100%;background:transparent;border:none;color:var(--green);font-family:var(--mono);font-size:13px;outline:none;font-weight:700;}
  .mesh-target-list{background:var(--bg2);border:1px solid var(--border);border-radius:3px;overflow:hidden;}
  .mesh-target-row{display:grid;grid-template-columns:28px 1fr 60px 60px;gap:6px;padding:6px 10px;align-items:center;border-bottom:1px solid rgba(255,255,255,0.04);}
  .mesh-target-row.active{background:rgba(0,255,136,0.06);border-left:2px solid var(--green);}
  .mesh-target-row.in-interval{background:rgba(0,191,255,0.06);border-left:2px solid var(--cyan);}
  .mesh-bar-mini{height:4px;background:var(--border);border-radius:2px;overflow:hidden;}
  .mesh-bar-mini-fill{height:100%;border-radius:2px;transition:width 0.4s;}
  .mesh-expectancy-meter{padding:10px;background:var(--bg2);border:1px solid var(--border);border-radius:3px;}
  .mesh-span-badge{display:inline-flex;align-items:center;padding:3px 10px;border-radius:2px;font-size:11px;font-weight:700;letter-spacing:1px;}
  .mesh-span-ok{background:var(--green-dim);border:1px solid var(--green);color:var(--green);}
  .mesh-span-warn{background:var(--red-dim);border:1px solid var(--red);color:var(--red);}
  .mesh-log{font-family:var(--mono);font-size:9px;color:var(--text-dim);padding:8px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:3px;min-height:36px;line-height:1.7;white-space:pre-wrap;}
  .mesh-matrix-grid{display:grid;grid-template-columns:repeat(11,1fr);gap:1px;font-size:8px;margin-top:8px;}
  .mesh-matrix-cell{padding:3px 1px;text-align:center;border-radius:1px;}
  .mesh-matrix-header{color:var(--text-dim);font-weight:700;}
...
export default function DerivOracle() {
  ...
  // Auto AI every 25 new ticks
        ...
        // ---- MESH ENGINE: feed digit on every live tick ----
        const meshNewDigit = parseInt(parseFloat(price).toFixed(2).slice(-1));
        updateMeshEngine(meshNewDigit);
            ...
            <div className="tabs-wrapper">
              ...
              <div className="tabs-row">
                ...
                <div className="tab-group">
                  ...
                  {[["signals","Signals"],["predict","Predict"],["under5","Under 5"],["lab","Lab"],["mesh","Mesh"]].map(([id,label]) => (
                    <button key={id} className={"tab"+(activeTab===id?" active":"")} onClick={()=>setActiveTab(id)}>{label}</button>
                  ))}
                </div>
                <div className="tab-group">
            ...
            <div>
              ...
              <div className="mesh-config-row">
                ...
                <div className="mesh-config-item">
                  ...
                  <select className="mesh-config-input" value={meshK}
                    onChange={e => { setMeshK(parseInt(e.target.value)); resetMeshEngine(); }}
              ...
              <div className="mesh-layout">
                ...
                <div className="mesh-canvas-wrap" style={{minHeight:420}}>
                  ...
                  <div style={{padding:"8px 12px", borderTop:"1px solid var(--border)"}}>
                    <div className="mesh-log">{meshLog}</div>
                  </div>
                </div>
